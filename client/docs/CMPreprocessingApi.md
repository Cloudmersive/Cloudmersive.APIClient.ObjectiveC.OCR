...
[**preprocessingUnrotateAdvanced**](CMPreprocessingApi.md#preprocessingunrotateadvanced) | **POST** /ocr/preprocessing/image/unrotate/advanced | Detect and unrotate a document image (advanced)
...
// Detect and unrotate a document image (advanced)
[apiInstance preprocessingUnrotateAdvancedWithImageFile:imageFile
          completionHandler: ^(NSData* output, NSError* error) {
                        if (output) {
                            NSLog(@"%@", output);
                        }
                        if (error) {
                            NSLog(@"Error calling CMPreprocessingApi->preprocessingUnrotateAdvanced: %@", error);
                        }
                    }];
```
